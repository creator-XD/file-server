...
Если тело запроса не является корректным JSON:
...
Если путь содержит небезопасные части вроде `../`:
...
Если папка уже существует, сервер вернет `409 Conflict`:
...
Если папка не найдена, сервер вернет `404 Not Found`:
...
Если путь небезопасный, например содержит `../`, сервер вернет `400 Bad Request`.
...
- пути директорий нормализуются, `.` и `..` запрещены
